...
class Toggle extends React.Component {
  ...
  /**
   * Method to handle when the switch is clicked
   * pdates the `on` state and calls the `onToggle` prop
   * with the new `on` state.
   * 💰 this.setState(newState, callback)
   */
  toggle = () => {
    this.setState(
      //function 1 (newstate)(updater function)
      ({on}) => ({on: !on}),

      //function 2 (callback)(something to do after)
      () => {
